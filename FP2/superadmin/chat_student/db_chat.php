<?php
    $connect = new PDO("mysql:host=localhost;dbname=ayomagang;charset=utf8mb4", "root", "");
    ...
    function fetch_user_chat_history($from_user_id, $to_user_id, $connect){
        ...
        foreach($result as $row){
            ...
            $output .= '
                    ...
                    <div align="right">- <small><em>'.$row['timestamp'].'</em></small></div></p></li>';
        }
        $output .= '</ul>';
    ...
    function fetch_group_chat_history($connect){
        ...
        foreach($result as $row){
            ...
            $output .= '
                    ...
                    <p>'.$user_name.' - '.$chat_message.' 
                    <div align="right">
                    - <small><em>'.$row['timestamp'].'</em></small></div></p></li>';
        }
        $output .= '</ul>';
        return $output;
    }
    ...
    function get_user_name($user_id, $connect){
        $query = "SELECT username FROM superadmin WHERE superadmin_id = '$user_id'";
        $statement = $connect->prepare($query);
        $statement->execute();
        $result = $statement->fetchAll();
    ...
    function fetch_is_type_status($user_id, $connect){
        ...
        $output = '';
        foreach($result as $row){
            if($row["is_type"] == 'yes'){$output = ' - <small><em><span class="text-muted">mengetik...</span></em></small>';}
        }
        return $output;
    }
?>