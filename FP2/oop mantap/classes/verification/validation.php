...
class validation extends db {
    ...
    public function validate($field, $label, $rules){
        ...
        if(in_array("required", $allRules)){
            if(empty($inputField)){return $this->errors[$field] = $label . " is required";}
        }
        // Close required rule
        ...
        if(in_array('uniqueEmail', $allRules)){
            ...
            if($result->execute([$inputField])){
                if($result->rowCount() > 0 ){return $this->errors[$field] = $label . " is already exist";}
            }
        }
        ...
        if(in_array("min_len", $allRules)){
            ...
            if(strlen($inputField) < $minValue){return $this->errors[$field] = $label . " is too short";}
        }
    }
    public function run(){
